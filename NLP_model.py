...
def TF_data(xtrain, xvalid):
    tfv = TfidfVectorizer(min_df=3,  max_features=15000,
...
def count_data(train, test):
    ...
    # Number of stopwords
    df["count_stopwords"] = df["comment_text"].apply(lambda x: len(
    ...
    train_feats['count_unique_word'].loc[
        train_feats['count_unique_word'] > 200] = 200
    SELECTED_COLS = ['count_sent', 'count_word', 'count_unique_word',
                     'count_letters', 'count_punctuations', 'count_words_upper',
                     'count_words_title', 'count_stopwords', 'mean_word_len',
                     'word_unique_percent', 'punct_percent']
    target_x = train_feats[SELECTED_COLS]
    target_y = test_feats[SELECTED_COLS]
    return target_x, target_y


def LR_model(xtrain_tfv, ytrain, xvalid_tfv, yvalid):
    clf = LogisticRegression(
        C=1.0, solver='sag', dual=True, random_state=520, class_weight="balanced")
...
def clean(comment):
    ...
    clean_sent = u" ".join(words).encode('utf-8')
