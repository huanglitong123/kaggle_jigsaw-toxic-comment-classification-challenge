...
def runLGB(train_X, train_y, test_X, test_y=None, feature_names=None, seed_val=2017, num_rounds=400):
    param = {
        'boosting_type': 'gbdt',
        'application': 'binary',
        'metric': 'auc',
    ...
    if test_y is not None:
        xgtest = lgb.Dataset(test_X, label=test_y)
        watchlist = [(xgtrain, 'train'), (xgtest, 'test')]
        model = lgb.train(params=param, train_set=xgtrain,
                          num_boost_round=num_rounds)
    else:
        xgtest = lgb.Dataset(test_X)
        model = lgb.train(params=param, train_set=xgtrain,
                          num_boost_round=num_rounds)
...
for i, j in enumerate(col):
    print('fit '+j)
    model = runLGB(comments_train, train_l[j], comments_valid, valid_l[j])
    preds[:, i] = model.predict(lgb.Dataset(comments_test))
    gc.collect()

subm = pd.read_csv('./data/sample_submission.csv')
submid = pd.DataFrame({'id': subm["id"]})
submission = pd.concat([submid, pd.DataFrame(preds, columns=col)], axis=1)
submission.to_csv('lgb_simple.csv', index=False)
